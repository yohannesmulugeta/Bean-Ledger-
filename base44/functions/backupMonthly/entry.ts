...
Deno.serve(async (req) => {
  ...
  const [purchases, receipts, processingLogs, outputReports, activityLogs] = await Promise.all([
    base44.asServiceRole.entities.PurchaseRecord.filter({ archived: false }, '-purchase_date', 5000),
    base44.asServiceRole.entities.WarehouseReceipt.filter({ archived: false }, '-received_date', 5000),
    base44.asServiceRole.entities.ProcessingLog.filter({ archived: false }, '-date', 5000),
    base44.asServiceRole.entities.OutputReport.filter({ archived: false }, '-date', 5000),
  ...
  // Sheet 1: Purchase Summary (all time)
  ...
  // Sheet 2: Payments Report (all time)
  const paymentRows = [];
  ...
  // Sheet 4: Warehouse Stock (all time)
  ...
  // Sheet 5: Processing Log (all time)
  ...
  // Sheet 6: Output Report (all time)
  ...
  // Sheet 7: Activity Log (this month)
  const activityRows = activityLogs
    .filter(a => new Date(a.created_date) >= startOfMonth)
    .map(a => ({
  ...
  const filename = `KKGT-Monthly-Backup-${monthName}-${yyyy}.xlsx`;
