...
Deno.serve(async (req) => {
  ...
  for (const p of purchases) {
    ...
    if (!supplierMap[name]) {
      supplierMap[name] = { supplier: name, grandTotal: 0, totalPaid: 0, balance: 0 };
    }
  ...
  // Sheet 7: Activity Log (last 24h)
  const activityRows = activityLogs
    .filter(a => new Date(a.created_date) >= since24h)
    .map(a => ({
  ...
  const filename = `KKGT-Daily-Backup-${dd}-${mm}-${yyyy}.xlsx`;
